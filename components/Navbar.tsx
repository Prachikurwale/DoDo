...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-white p-4 flex justify-between items-center relative z-50 shadow-sm">
      {/* DoDo Logo */}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button 
            onClick={() => setIsOpen(!isOpen)}
            className="w-12 h-12 rounded-full overflow-hidden border-2 border-pink-200 shadow-md flex items-center justify-center bg-pink-400 text-white font-bold text-xl"
          >
            {user?.image ? (
              <img src={user.image} alt="profile" className="w-full h-full object-cover" />
            ) : (
          ...
          <AnimatePresence>
            {isOpen && (
              ...
              >
                <div className="w-20 h-20 rounded-full bg-pink-400 flex items-center justify-center text-white text-3xl font-bold shadow-lg overflow-hidden">
                  {user?.image ? <img src={user.image} className="w-full h-full object-cover" /> : initial}
                </div>
                <div className="text-center">
                  <p className="font-black text-gray-800 text-lg">{user?.name}</p>
                ...
                <button 
                  onClick={() => signOut()}
                  className="w-full flex items-center justify-center gap-2 bg-gray-50 hover:bg-red-50 text-gray-700 hover:text-red-600 py-3 rounded-2xl transition-colors font-bold"
                >
                  <LogOut size={18} />
                  Sign Out
                </button>
